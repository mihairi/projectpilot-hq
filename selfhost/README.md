...
Deadline, status-change and priority-change notifications are written to the
notifications table by database triggers. To have them delivered by mail as well, run a
small scheduled script on the server that selects rows not yet emailed, sends each one
through the same Exchange Online credentials with any SMTP client (for example
`nodemailer`), and marks them as sent. Everything it needs — recipient address from the
profile, title, body, and the link back into the app — is already stored on each row.
